...
class FileUtils {
    ...
    private final val TAG = "FileUtils"
    private final val SCALE_FACTOR = 50.0
    private final val BUFFER_SIZE = 1024 * 1024 * 8
    private final val DEGREES_90 = 90
    private final val DEGREES_180 = 180
    private final val DEGREES_270 = 270
...
//        }
        ...
        val albumDir = getAlbumFolderPath(folderName, MediaType.Image, albumType)?.let { File(it) }
        ...
        if (android.os.Build.VERSION.SDK_INT < 29) {
            ...
            values.put(MediaStore.Images.Media.RELATIVE_PATH, albumType + File.separator + folderName)
        }
...
//        }
        ...
        val albumDir = getAlbumFolderPath(folderName, MediaType.Video, albumType)?.let { File(it) }
        ...
        if (android.os.Build.VERSION.SDK_INT < 29) {
            ...
            values.put(MediaStore.Video.Media.RELATIVE_PATH, albumType + File.separator + folderName)
        }
    ...
    private fun getAlbumFolderPath(
        ...
        var albumFolderPath: String?  = context.getExternalFilesDir(albumType)?.path
        //var albumFolderPath: String = Environment.getStorageDirectory().path
        //toDcim &&
//        if ( android.os.Build.VERSION.SDK_INT < 29) {
//            albumFolderPath += File.separator + albumType ; //Environment.DIRECTORY_DCIM
//        }

        albumFolderPath = if (TextUtils.isEmpty(folderName)) {

            val baseFolderName = albumType ;
...
//            }
            ...
            context.getExternalFilesDir(baseFolderName)?.let {
                createDirIfNotExist(
                    it.path
                )
            } ?: albumFolderPath
