...
class MediaGallerySaver internal constructor(private val activity: Activity) :
    ...
    internal fun checkPermissionAndSaveFile(
        ...
        albumType  = methodCall.argument<Any>(ALBUM_TYPE) as String
        ...
        uiScope.launch {
            val success = async(Dispatchers.IO) {
                if (mediaType == MediaType.Video) {
                    ...
                    FileUtils().insertVideo(activity.contentResolver, filePath, albumName, albumType)
                } else {
                    FileUtils().insertImage(activity.contentResolver, filePath, albumName, albumType)
                }
            }
            success.await()
            finishWithSuccess()
    ...
    companion object {
        ...
        private  const val ALBUM_TYPE = "albumType"
    }
